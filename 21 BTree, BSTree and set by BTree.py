...
def second(t):
    ...
    if t.is_leaf():
        return None
    elif t.right is BTree.empty: # 说明该right是global max
        return largest(t.left) # 很tricky: 在其left找2nd max
    
    elif t.right.is_leaf(): # 只有上述不成立时
        return t.label
    else:
...
# Sets as binary search trees: 主要是addjoin 要注意去重

# Binary Search !! 牢记只会走一条路，left or right!!!不要忘记，总去考虑全部遍历！
def contains(s, v):
    ...
    # base case: 遍历到了leaf！
    # print("tree", repr(s))
    if s is BTree.empty:# 说明上一次，s是leaf, s.left/s.right是None
        return False
    # check label
    elif s.label == v: 
        return True
...
def adjoin(s, v):
    ...
    """
    if s is BTree.empty: # 说明上一次，s是leaf, s.left/s.right是None
        return BTree(v) #别忘记了，就算是只有label的branch(node)都是BTree tpye 而非int!
    elif s.label > v: # put into left branch
